<?php 
	...
	function addUser($conn, $username, $password) {
		$sql = "SELECT * FROM user_account WHERE username=?"; // Query to check if username already exists
		...
		if($stmt->rowCount() == 0) {
			$sql = "INSERT INTO user_account (username,password) VALUES (?,?)"; // Query to insert new user
			$stmt = $conn->prepare($sql);
	...
	function login($conn, $username, $password) {
		$query = "SELECT * FROM user_account WHERE username=?"; // Query to retrieve user information
